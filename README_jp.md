...
Installed folder watcher windows service works in background and **renames any new csv file within hardcoded directory** based on parent directory name.(インストールされた folder watcher windows service は バックグラウンドで動作し、ハードコードされたディレクトリ内の親ディレクトリ名に基づいて csv ファイル名を変更します)
...
(監視しているフォルダーのパスは、`RenameFolderService.py` でハードコードされていることに注意してください)
...
`net start RenameFolderService`

To stop service, in command line

`net stop RenameFolderService`
